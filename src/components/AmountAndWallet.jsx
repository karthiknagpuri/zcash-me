    ...
    <div className="w-full mb-2">
      <div className="flex items-center gap-3">
        <div className="relative flex flex-1 items-stretch overflow-visible">
          {showUsdPill && (
            <>
              <div
                className="pointer-events-none absolute top-0 border-t border-[#f5c542]/50"
                style={{
                  width: overlayWidth,
              ...
              />
              <div
                className="pointer-events-none absolute bottom-0 border-b border-[#f5c542]/50"
                style={{
                  width: overlayWidth,
                  right: overlayRightOffset
                }}
          ...
          >
            <input
              ...
              className="border border-[#f5c542]/50 bg-[#1a1a1a] px-3 rounded-xl w-full h-10
                         text-sm pr-16 text-[#faf6ed] placeholder-[#faf6ed]/40
                         pl-3"
            />

            {/* Right-side selector */}
            <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center text-[#faf6ed]/60 text-sm select-none cursor-not-allowed">
              ZEC ▼
            </div>
          </div>
          ...
          {showUsdPill && (
            <div
              className={`relative flex items-center border border-l-0 border-[#f5c542]/50 bg-[#1a1a1a] rounded-r-xl text-[#faf6ed]/60 text-sm h-10 overflow-visible min-w-0 transition-[width] duration-200 box-border ${
            ...
            >
              ...
              >
                <span
                  className="text-[#faf6ed]/60 cursor-pointer flex-none hover:text-[#f5c542]"
                ...
                {isUsdOpen && (
                  <>
                    <input
                      ...
                      className="min-w-0 flex-1 bg-transparent text-left tabular-nums text-[#faf6ed]/60 focus:outline-none disabled:opacity-60"
                    />
                    <div className="ml-2 flex items-center gap-1 text-[#faf6ed]/60 shrink-0">
                      <span>{fiat}</span>
                      <span
                        className="cursor-pointer hover:text-[#f5c542]"
                        role="button"
        ...
        {showOpenWallet && (
          <button
            onClick={openWallet}
            className="flex items-center gap-1 border rounded-xl px-3 py-2 text-sm transition-all duration-200 border-[#f5c542] hover:bg-[#f5c542] text-[#faf6ed] hover:text-[#1a1a1a] whitespace-nowrap"
          >
            Open in Wallet
          </button>
        )}
      ...
      {showRateMessage && (
        <div className="w-full flex items-center justify-center gap-2 text-center mt-2 min-h-[18px]">
          <p
            className={`text-[10px] italic m-0 text-[#faf6ed]/50 transition-all duration-200 ${
