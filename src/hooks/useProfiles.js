...
import { getMockProfilesWithLinks } from "../../lib/mock-data";
  ...
  useEffect(() => {
    ...
    async function fetchAllProfiles() {
      ...
      try {
        ...
        if (process.env.NODE_ENV === "development" && enriched.length === 0 && isLocalDemo()) {
          enriched = getMockProfilesWithLinks();
          console.warn("Supabase unavailable - using mock data for local demo");
        }
        ...
        if (process.env.NODE_ENV === "development" && active && isLocalDemo()) {
          const fallback = getMockProfilesWithLinks();
          cachedProfiles = fallback;
          if (typeof window !== "undefined") window.cachedProfiles = fallback;
          setProfiles(fallback);
          if (!hasInitial) setLoading(false);
          console.warn("Using mock data for local demo due to fetch error");
        }
      }
    }
